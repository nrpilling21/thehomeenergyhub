...
function markdownToHtml(md: string): string {
  return md
    ...
    .map(block => {
      ...
      if (!block) return '';
      if (block.startsWith('### '))
        return `<h3 class="text-lg font-bold mt-8 mb-3">${block.slice(4)}</h3>`;
      if (block.startsWith('## '))
        return `<h2 class="text-xl font-bold mt-10 mb-4">${block.slice(3)}</h2>`;
      if (block.match(/^[-*] /m)) {
        const items = block
          .split('\n')
          .filter(l => l.match(/^[-*] /))
          .map(l => `<li class="mb-1">${l.replace(/^[-*] /, '')}</li>`);
      ...
      <article className="max-w-3xl mx-auto px-5 py-12">
        ...
        <div className="bg-green-50 border border-green-200 rounded-2xl p-8 mt-12 text-center">
          ...
          <p className="text-gray-500 text-sm mb-4">
            Try our free calculators &mdash; no email required.
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            <a
              href="/heat-pump-cost-calculator"
              className="px-6 py-3 bg-green-700 text-white rounded-xl font-semibold text-sm hover:bg-green-800 transition"
            >
              Heat Pump Calculator
            </a>
            <a
              href="/ev-charging-cost-calculator"
              className="px-6 py-3 bg-white border border-green-700 text-green-700 rounded-xl font-semibold text-sm hover:bg-green-50 transition"
            >
